...
def delete(): #need to tweak and check why not working
    index = user_data_listbox.curselection()[0]
    with open('user_data.csv') as f:
        reader = csv.reader(f)
        data_list = list(reader)
        delete_elem = data_list[index+1]
        
    rewrite_csv = list()
    with open('user_data.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            rewrite_csv.append(row)
            for element in row:
                if element == delete_elem:
                    rewrite_csv.remove(row)
...
delete_button = Button(root,text="Delete",command=delete, padx= 10 ).grid (row=5, column=2)
show_pass_button = Button(root,text="Show Password", command=show_password).grid (row=5, column=1)



#bug found where the generate only shows capital letters when the radio button is left unchecked
